...
def poisson3d(*, source, xfaces, yfaces, zfaces, xtype = 'dirichlet', ytype = 'dirichlet', ztype = 'dirichlet', tolerance = 1E-5, check = 20, itermax = 1E5):
...
def correct3d(*, delp, u, v, w, xfaces, yfaces, zfaces, xtype = 'dirichlet', ytype = 'dirichlet', ztype = 'dirichlet'):
    ...
    if ztype == 'dirichlet':
        ...
        w[ 0,:,:] = w[ 0,:,:] - ddys[ 0,None,None] * (delp[0,:,:] - delp[-1,:,:])
        w[-1,:,:] = w[-1,:,:] - ddyn[-1,None,None] * (delp[0,:,:] - delp[-1,:,:])
    else:
        pass
