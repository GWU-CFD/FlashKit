...
def author_xdmf(filenames: dict[str, str], filesteps: Sequence[int], context: Bar) -> ElementTree.Element:
    ...
    with context(len(filesteps)) as progress:
        for step, number in enumerate(filesteps):
            ...
            gridsource = first_true((f'{filenames["grid-source"]}{file:04}' for file in reversed(filesteps)), os.path.exists)
            griddest = first_true((f'{filenames["grid-dest"]}{file:04}' for file in reversed(filesteps)), os.path.exists)
